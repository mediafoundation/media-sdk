...
class MarketplaceViewer {
    ...
    constructor() {
        ...
        if (addresses.MarketplaceViewer[this.config.publicClient.chain.id] === undefined) {
    ...
    async getAllOffersPaginating(marketPlaceId, start = 0, steps = 20) {
        ...
        let result = await this.view('getAllOffers', [marketPlaceId, _start, _steps])
        offers.push(...result[0])

        if (result[1] > offers.length) {
            let totalOffers = result[1]
            for (let i = BigInt(1); i * _steps < totalOffers; i++) {
                let result = await this.view('getAllOffers', [marketPlaceId, _start + i * _steps, _steps])
                offers.push(...result[0])
            }

            if (totalOffers > offers.length) {
                let result = await this.view('getAllOffers', [marketPlaceId, _start + totalOffers, totalOffers - BigInt(offers.length)])
                offers.push(...result[0])
            }
        }
    ...
    async getPaginatedDeals(
        ...
        return await this.view('getPaginatedDeals', [marketPlaceId, userAddress, isProvider, start, steps])
    }
    ...
    async getAllDealsPaginating(
        ...
        let result = await this.view('getPaginatedDeals', [marketPlaceId, userAddress, isProvider, _start, _steps])
        deals.push(...result[0])

        if (result[1] > deals.length) {
            let totalDeals = result[1]
            for (let i = BigInt(1); i * _steps < totalDeals; i++) {
                let result = await this.view('getPaginatedDeals', [marketPlaceId, userAddress, isProvider, _start + i * _steps, _steps])
                deals.push(...result[0])
            }

            if (totalDeals > deals.length) {
                let result = await this.view('getPaginatedDeals', [marketPlaceId, userAddress, isProvider, _start + totalDeals, totalDeals - BigInt(deals.length)])
                deals.push(...result[0])
            }
        }
